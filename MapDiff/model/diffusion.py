...
class DiscreteMarginalTransition:
    def __init__(self, x_classes=20, x_marginal_path='data/train_magrinal_x.pt'):
        self.X_classes = x_classes
        self.x_marginal = torch.load(x_marginal_path, map_location='cpu')
...
class BlosumTransition:
    def __init__(self, blosum_path='data/blosum_substitute.pt', x_classes=20, timestep=500):
        try:
            blosum_data = torch.load(blosum_path, map_location='cpu')
            self.original_score, self.temperature_list, self.Qt_temperature = blosum_data['original_score'], \
                blosum_data['Qtb_temperature'], blosum_data['Qt_temperature']
