from .utils import normalize, safe_index, one_hot_res, log, dihedral, NormalizeProtein, dataset_argument, get_stat
import argparse
...
class Cath_imem(InMemoryDataset):
    ...
    def get_calpha_graph(self, rec, c_alpha_coords, n_coords, c_coords, o_coords, cb_coords, coords,
                         struc_2nd_res=None):
        # scalar_feature: sin-cos dihedral
        ...
        atom_pos = np.stack(atom_pos, axis=1)
        ...
        if x is not None:
            ...
            # edge_feat_ori_feat: local frames
            edge_feat_ori_list = []
    ...
    def get_receptor_inference(self, rec_path):
        ...
        for i, chain in enumerate(rec):
            ...
            for res_idx, residue in enumerate(chain):
                ...
                if c_alpha != None and n != None and c != None:
                    ...
                    if o is None:
                        chain_o_coords.append(list(np.zeros(3)))
                        chain_o_mask.append(0)
                    else:
                        chain_o_coords.append(o)
                        chain_o_mask.append(1)
                    if cb is None:
                        chain_cb_coords.append(list(np.zeros(3)))
                        chain_cb_mask.append(0)
                    else:
                        chain_cb_coords.append(cb)
                        chain_cb_mask.append(1)
            ...
            cb_coords.append(np.array(chain_cb_coords))
            o_mask.append(np.array(chain_o_mask))
            cb_mask.append(np.array(chain_cb_mask))
        ...
        for i, chain in enumerate(rec):
            if chain.get_id() in valid_chain_ids:
                ...
                valid_cb_coords.append(cb_coords[i])
                valid_o_mask.append(o_mask[i])
                valid_cb_mask.append(cb_mask[i])
        ...
        cb_coords = np.concatenate(valid_cb_coords, axis=0)  # [n_residues, 3]
        o_mask = np.concatenate(valid_o_mask, axis=0)
        cb_mask = np.concatenate(valid_cb_mask, axis=0)

        assert o_coords[o_mask == 0].sum() == 0
        assert cb_coords[cb_mask == 0].sum() == 0

        o_mask = torch.tensor(o_mask, dtype=torch.bool)
        cb_mask = torch.tensor(cb_mask, dtype=torch.bool)
        ...
        assert len(c_alpha_coords) == len(cb_coords)
        assert sum(valid_lengths) == len(c_alpha_coords)
        return rec, coords, c_alpha_coords, n_coords, c_coords, o_coords, cb_coords, o_mask, cb_mask
