...
class WetLab_Landscape:
    def __init__(self, oracle_model, wt_accession, selected_residue_idx, wt_seq, target_value, sigma=1.0):
        ...
        self.wt_pas = self.pas(seq_data[0].label)
    ...
    def pas(self, pred_value):
        delta = abs(pred_value - self.target_value)
        pas_score = math.exp(-(delta ** 2) / (2 * self.sigma ** 2))
        return round(pas_score, 2)

    def get_pas(self, mutated_residues):
        ...
        pas_scores = [self.pas(_) for _ in mutated_seq_data_labels]
        return pas_scores

    def get_wt_fitness(self):
        return self.get_wt_pas()

    def fitness(self, pred_value):
        return self.pas(pred_value)

    def get_fitness(self, mutated_residues):
        return self.get_pas(mutated_residues)
