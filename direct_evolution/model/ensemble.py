import numpy as np

ensemble_rules = {
    'mean': lambda x: np.mean(x, axis=0),
    'lcb': lambda x: np.mean(x, axis=0) - np.std(x, axis=0),
    'ucb': lambda x: np.mean(x, axis=0) + np.std(x, axis=0)
}

class Ensemble:
    def __init__(self, models, ensemble_rule):
        self.models = models
        self.ensemble_func = ensemble_rules[ensemble_rule]
    
    def train(self, sequences, labels):
        for model in self.models:
            model.train(sequences, labels)

    def get_pas(self, sequences):
        # Input:  - sequences:   [batch_size, sequence_length]
        # Output: - predictions: [batch_size]
        
        return self.ensemble_func([model.get_pas(sequences) for model in self.models])

    def get_fitness(self, sequences):
        return self.get_pas(sequences)

    def forward(self, y, rule='ucb'):
        """
            x, y: bsz x seq_len x vocab_sz
            return: [bsz] x count
        """
        return ensemble_rules[rule]([model(y).tolist() for model in self.models])
