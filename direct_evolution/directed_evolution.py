...
def run(config_de):
    ...
    data_store = dict({'step':[], 'sequence': [], 'pas': [], 'distance': []})
    ...
    wt_pas = oracle.get_wt_pas()
    data_store['step'].append(0)
    data_store['sequence'].append(selected_residues)
    data_store['pas'].append(wt_pas)
    data_store['distance'].append(0)
    logger.info(f'wt PAS: {wt_pas}')

    sample_model = SampleModel(wt_pas, config_de, psi_model_ensemble)

    seq_scores = dict({selected_residues: wt_pas})
    max_pas = wt_pas
    ...
    for i in range(1, config_de.evo_steps + 1):
        ...
        prev_max_pas = max_pas
        seed_seqs = [max_seq]
        for bi in tqdm(range(0, len(samples), config_de.oracle_batch_size)):
            sample_batch = samples[bi:bi + config_de.oracle_batch_size]
            pas_scores = oracle.get_pas(sample_batch)  # bsz x 1
            samples_pas += pas_scores

            batch_max = np.max(pas_scores)
            if  batch_max > max_pas:
                max_pas = batch_max
                max_seq = sample_batch[np.argmax(pas_scores)]

            for si in range(len(sample_batch)):
                seq_scores[sample_batch[si]] = pas_scores[si]
                samples_seq_scores[sample_batch[si]] = pas_scores[si]
                if pas_scores[si] > prev_max_pas:
                    seed_seqs.append(sample_batch[si])

        all_seqs.update(samples)
        logger.info('{} sampled sequences PAS: max: {:.4f}, min: {:.4f}, avg: {:.4f}'.format(len(samples_pas), np.max(samples_pas), np.min(samples_pas), np.mean(samples_pas)))

        logger.info('global measured sequences count: {}'.format(len(all_seqs)))
        logger.info('global max PAS: {:.4f}, distance to wt: {}'.format(max_pas, hamming_distance(selected_residues, max_seq)))

        data_store['step'] += [i for j in range(len(samples))]
        data_store['sequence'] += samples
        data_store['pas'] += samples_pas
        data_store['distance'] += [hamming_distance(selected_residues, seq) for seq in samples]

        if i == config_de.evo_steps:
            if getattr(config_de, 'save_surrogate_models', False):
        ...
        for mi, psi_model in enumerate(psi_model_ensemble.models):
            ...
            logger.info(f"fine-tuning PAS model {mi}:")
            epoch_losses = []
            patience = 0
            min_loss = torch.inf
            epoch = 1
