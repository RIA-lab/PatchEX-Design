...
from models.loss_func import WeightedRMSELoss, TemperatureRangeLoss
import torch.nn.functional as F


class PatchETPretrained(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, labels=None):
        ...
        intra_hidden_state = hidden_state
        ...
        for i in range(self.patch_inter_heads):
            ...
            global_hidden_state = patch_inter_values * weights
            x_sum = torch.sum(global_hidden_state, dim=-1)  # (B, D')
            x_max, _ = torch.max(global_hidden_state, dim=-1) # (B, D')
            if i == 0:
                cat_xsum = x_sum
                cat_xmax = x_max
            else:
        ...
        if self.inference:
            return ModelOutput(pred=pred, intra_hidden_state=intra_hidden_state, global_hidden_state=global_hidden_state)

        loss = self.loss_fct(pred, labels) if labels is not None else None
        return ModelOutput(loss=loss, pred=pred)


class PatchETRangePretrained(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.patch_inter_heads = config['n_patch_inter_heads']
    ...
    def forward(self, input_ids, attention_mask, labels=None):
        ...
        for i in range(self.patch_inter_heads):
            weights = F.softmax(self.patch_inter_layers[i](hidden_state.transpose(1, 2)), dim=-1)
            global_hidden_state = patch_inter_values * weights
            x_sum = torch.sum(global_hidden_state, dim=-1)  # Sum pooling
            x_max, _ = torch.max(global_hidden_state, dim=-1)
            if i == 0:
                cat_xsum = x_sum
                cat_xmax = x_max
            else:
