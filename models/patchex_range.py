...
from .patchet_pretrained import PatchETRangePretrained
import yaml
...
class PatchPredHead(nn.Module):
    ...
    def forward(self, local_feats, global_feats, return_attn=False):
        ...
        patch_preds = self.mlp(final)        # [B, P, 2]
...
class Model(nn.Module):
    def __init__(self, config):
        ...
        if config['pretrained_patchet'] is not None:
            ...
            self.patchet = PatchETRangePretrained(patchet_pretrained_config)
    ...
    def _compute_joint_loss(self, global_pred, patch_preds, labels, patch_mask):
        ...
        global_pred: [B,2] (low, high)
        patch_preds: [B,P,2] (low, high)
        labels: [B,2] (low, high)
        ...
        # reshape for range prediction [B, 2] and [B, P, 2]
        gp = global_pred  # [B, 2]
        pp = patch_preds  # [B, P, 2]
        y = labels  # [B, 2]
        ...
        # Calculate MSE for both low and high predictions [B, 2]
        per_sample_mse_range = (gp - y).pow(2)  # [B, 2]

        # Sum MSE over the range dimensions (low, high) to get per-sample loss [B]
        per_sample_mse = per_sample_mse_range.sum(dim=-1)  # [B]

        # Focal weights calculation needs to be adjusted for range (using L1 error across the range)
        if self.focal_gamma > 0.0:
            abs_err_range = torch.abs(gp - y).detach()  # [B, 2]
            abs_err = abs_err_range.sum(dim=-1)  # Sum L1 error for low and high [B]
        ...
        # 2) per-element squared error [B,P,2] for patches
        # Expand labels: [B, 2] -> [B, 1, 2] -> [B, P, 2]
        labels_expanded = y.unsqueeze(1).expand_as(pp)  # [B, P, 2]

        if self.patch_loss_type == 'mse':
            # per_patch_sq_range: [B, P, 2]
            per_patch_sq_range = (pp - labels_expanded).pow(2)
        elif self.patch_loss_type == 'huber':
            # F.smooth_l1_loss takes B,P,2 and B,P,2, returns B,P,2 (reduction='none')
            per_patch_sq_range = F.smooth_l1_loss(pp, labels_expanded, reduction='none')
        else:
            raise ValueError("patch_loss_type must be 'mse' or 'huber'")

        # Sum over the range dimension: [B, P, 2] -> [B, P] (loss for a patch is the sum of low/high losses)
        per_patch_sq = per_patch_sq_range.sum(dim=-1)  # [B,P]
        ...
        # Use the summed L1 error for gating: [B]
        global_error = torch.abs(gp - y).sum(dim=-1)  # [B]
        ...
        if self.lambda_consistency > 0:
            # Consistency loss calculation uses L1 error across range (like gating)
            errors_range = torch.abs(pp - labels_expanded)  # [B,P,2]
            errors = errors_range.sum(dim=-1)  # [B,P]
            ...
            # Weighted aggregation needs to be done across the range dimension as well
            # [B,P] * [B,P,2] -> [B,P,2] (element-wise) -> sum(dim=1) -> [B,2]
            weighted_agg = torch.sum(weights.unsqueeze(-1) * pp, dim=1)  # [B,2]
    ...
    def forward(self, input_ids, attention_mask, labels=None):
        if self.pretrained_patchet:
            ...
            # 2) run PatchET
            # PatchET must return: global_pred [B,2], patch_preds [B,P,2], and optionally other features
            global_pred, patch_preds, inter_hidden_state, intra_hidden_state, global_feature = self.patchet(
                hidden_state)

        # shapes
        patch_preds_s = patch_preds  # [B,P,2]
        global_pred_s = global_pred  # [B,2]
        ...
        if labels is not None and not self.inference:
            ...
            # Labels: [B, 2]. Errors: [B, P, 2]. Sum over range dim for [B, P] error per patch
            labels_expanded = labels.unsqueeze(1)  # [B, 1, 2]
            errors_range = torch.abs(patch_preds_s - labels_expanded)  # [B,P,2]
            errors = errors_range.sum(dim=-1)  # [B,P]
            ...
            patch_weights = weights.unsqueeze(-1)  # [B,P,1] (Scalar weight per patch)

            # store diagnostics for external logging (move tensors to cpu to avoid GPU retention)
            self.last_forward_info = {k: (v.detach().cpu().item() if torch.is_tensor(
                v) and v.numel() == 1 else v.detach().cpu().numpy() if torch.is_tensor(v) else v)
            ...
            # global_pred_s is [B, 2] for range prediction. Keep it as is.
            return ModelOutput(loss=total_loss, pred=global_pred_s)

        # Inference mode (or labels is None): compute weights w.r.t global_pred
        else:
            # Global pred: [B, 2]. Errors: [B, P, 2]. Sum over range dim for [B, P] error per patch
            global_pred_expanded = global_pred_s.unsqueeze(1)  # [B, 1, 2]
            errors_inf_range = torch.abs(patch_preds_s - global_pred_expanded)  # [B,P,2]
            errors_inf = errors_inf_range.sum(dim=-1)  # [B,P]
            ...
            weights_inf_exp = weights_inf  # [B,P] (This is the required patch_weights output)

            # Weighted aggregation needs to be done across the range dimension as well
            # [B,P] * [B,P,2] -> [B,P,2] (element-wise) -> sum(dim=1) -> [B,2]
            aggregated_from_patches = torch.sum(weights_inf.unsqueeze(-1) * patch_preds_s, dim=1)  # [B,2]
            ...
            # Return final prediction and patch_weights
